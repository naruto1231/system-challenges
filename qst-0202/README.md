...
本問はすでに簡易的な環境が整備されているフロントエンドと、データベースが用意されています。これらを利用し、バックエンドサーバーを実装することが求められます。

### フロントエンド環境の立ち上げ

フロントエンド環境の立ち上げには [Docker](https://docs.docker.com/) を使用します。以下の手順でフロントエンド環境を立ち上げてください。
...
郵便番号や住所などの情報はデータベース`app.db`に格納されています。データベースエンジンは[SQLite](https://www.sqlite.org/index.html)です。
...
またすでに`addresses`テーブルにはレコードが格納されていて、データの内容は`addresses.csv`で確認ができます。
